# Hybrid Real-Time Credit Card Fraud Detection System (AI Dataset Version)
...
**Note:** This version uses a newly generated **AI-synthesized dataset**, replacing the original Kaggle dataset, to improve model robustness and test generalization.
...
**This version uses a new database structure compared to the old system.** Before running the project, you must reset the database:

1. Open **phpMyAdmin** (usually at `http://localhost/phpmyadmin`).
2. Locate the old database named `fraud_detection`.
3. **Delete** the old `fraud_detection` database.
4. **Create a new database** with the exact same name: `fraud_detection`.
5. Import the new schema (if provided) or let the system auto-generate the tables on first run.

## How to Run

1. **Start XAMPP**
   Open XAMPP and start the **Apache** and **MySQL** modules. This powers the PHP backend and the new database used by the admin dashboard and SimplePay.
...
- XAMPP must be running before you open either the admin dashboard or the SimplePay app, since both depend on the PHP backend and the new MySQL database.
- The AI-generated dataset is designed to mimic real-world transaction patterns while introducing synthetic edge cases to better evaluate the hybrid model.
